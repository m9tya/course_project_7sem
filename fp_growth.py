...
#
#     @staticmethod
#     def generate_rules(input_csv: str, output_path: str, min_support: float = 0.1, min_lift: float = 1.0):
#         df = pd.read_csv(input_csv)
#
#         # Преобразование данных в бинарный формат для анализа
#         df_encoded = pd.get_dummies(df[['vendor_project', 'product', 'severity', 'required_action']],
#                                     prefix=['vendor', 'prod', 'sev', 'action'])
#
#         # Получение частых наборов
...
#                 f.write(f"Поддержка: {row['support']}, Уверенность: {row['confidence']}, Лифт: {row['lift']}\n\n")
#
#         print(f"Ассоциативные правила сохранены в файл {output_path}")
#
...
#         rules_list = list(Rules.itertuples(index=False, name=None))
#         random_rules = random.sample(rules_list, min(top_n, len(rules_list)))
